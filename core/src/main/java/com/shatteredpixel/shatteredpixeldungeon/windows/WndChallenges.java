...
public class WndChallenges extends WndTabbed {
	...
	private class ChallengeTab extends Component {
		...
		ChallengeTab(int checked) {
			...
			RenderedTextBlock title = PixelScene.renderTextBlock( M.L(TextChallenges.class, "traditional"), 12 );
...
//			boxes = new ArrayList<>();
			...
			for (int i=0; i < Challenges.NAME_IDS.length; i++) {
				...
				pos = cb.bottom();
				content.setSize(width, (int) pos + 16);
			}
		}
	...
	private class DlcTab extends Component {
		...
		DlcTab(int checked) {
			...
			createChildren();
			// 标题和普通挑战布局（同原WndChallenges）
			RenderedTextBlock title = PixelScene.renderTextBlock( M.L(TextChallenges.class, "expansion"), 12 );
			title.hardlight( TITLE_COLOR );
			title.setPos(
					(WIDTH - title.width()) / 2,
					(TTL_HEIGHT - title.height()) / 2
